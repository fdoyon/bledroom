...
use btleplug::api::{Central, CentralEvent, Characteristic, CharPropFlags, Manager as _, Peripheral, ScanFilter, WriteType};
...
async fn find_primary_uuid_for(adapter: &Adapter, prefix: &str) -> Result<Option<Uuid>> {
...
async fn main() -> Result<()> {
    ...
    for adapter in adapter_list.iter() {
        ...
        let light = wait_for_advertisement(&adapter, &target).await?;
        ...
        for w in writable {
            ...
            for cmd in &cmds {
                match cmd {
                    AppCommand::TurnOn => send_command(&light, &w, &LightsCommands::Power(true)).await?,
                    AppCommand::TurnOff => send_command(&light, &w, &LightsCommands::Power(false)).await?,
                    AppCommand::Pink => send_command(&light, &w, &LightsCommands::rgby_f32(01f32, 0f32, 0.2f32, 0.4f32)).await?,
                    AppCommand::Demo => do_demo(&light, &w).await?,
...
async fn do_demo(light: &impl Peripheral, w: &Characteristic) -> Result<()> {
...
async fn send_command(light: &impl Peripheral, characteristic: &Characteristic, cmd: &LightsCommands) -> Result<()> {
...
async fn wait_for_advertisement(adapter: &Adapter, target: &PeripheralId) -> Result<impl Peripheral> {
